...
export async function POST(request) {
  try {
    const { translatedText, appliedRules, referenceCorpus } =
      await request.json();
    ...
    if (appliedRules && appliedRules.length > 0) {
      const ruleList = appliedRules
        .map(
          (r) =>
            `- ${r.name}：已自动将 "${r.pattern || r.replaced}" 替换为 "${r.replaced}"（${r.note || ""}）`
        )
        .join("\n");
    ...
    if (referenceCorpus?.content) {
      ...
      const truncatedCorpus = referenceCorpus.content.slice(0, 3000);
      corpusHint = `\n\n用户上传了参考语料「${referenceCorpus.name}」，这是同领域的英文论文/学术文章。请在进行术语一致性检查和学术表达风格判断时，参考以下语料中的用词习惯和表达方式：\n\n---参考语料开始---\n${truncatedCorpus}\n---参考语料结束---\n\n审校要求：\n- 对比译文中的术语和表达是否与参考语料一致\n- 如果参考语料中有更地道的学术表达，请在建议中指出\n- 如果译文中的表达与参考语料明显不同，提示用户参考语料中的惯用表达\n- 不要直接覆盖用户译文，仅作为建议来源`;
    }

    const userMessage = `请审校以下英文译文：\n\n${translatedText}${ruleHint}${corpusHint}`;
